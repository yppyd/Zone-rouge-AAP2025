...
t_graph * graph_new(int size) { //Création de la matrice d'adjacence
    t_graph * g = mallo c(sizeof(t_graph)); //allocation mémoire pour t_graph
    g->size = size; //taille du graphe noté
    g->m = malloc(g->size * sizeof(t_bool *));
    assert(g->m != NULL);
    for (int i = 0; i < g->size; i++) {
        g->m[i] = malloc(g->size * sizeof(t_bool));
        assert(g->m[i] != NULL);
        for (int j = 0; j < g->size; j++) g->m[i][j] = FAUX; //nécessaire pour initialiser la matrice
    }
    return g;
}

void graph_free(t_graph * g) { //parcours tous les éléments pour les libérer
    if (g == NULL) return;
...
t_graph * graph_read_from_file(FILE * fp) { 
    ...
    t_graph * g = graph_new(size); //création du  graphe de taille size lu via le premier caractère
