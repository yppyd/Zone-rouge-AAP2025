...
// Fonction Principale (Modifiée pour gérer l'affichage)
...
void Recherche_recur(t_graph *g, t_vertex x, t_vertex y, FILE *out) {
    ...
    // 2. Création d'une pile locale pour stocker le résultat
    ...
    if (Recherche_recur_f(g, x, y, marking, &stack)) {
        ...
        // 4. Si trouvé : Affichage direct dans le fichier 'out'

        // La pile contient : Start -> ... -> Goal (grâce à l'empilement au retour)
    ...
    // Note : stack est vide ici si on a affiché, ou vide si rien trouvé (list_new retourne NULL)
...
int Kosaraju_1_recur(t_graph *g, int x, t_bool * marking, int * order, int step){
    if(marking[x] == FAUX){
        ...
        // --- Adaptation Listes : Parcours voisins ---
        t_node * curr = g->l[x];
        while (curr != NULL) {
