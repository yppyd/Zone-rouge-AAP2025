 ...
 * Version : MATRICE - Gestion propre des fichiers (fopen/fprintf)
...
// Note : Ajout de FILE *out pour l'affichage
...
int main(int argc, char *argv[]) {
    ...
    // 3. Gestion du flux de sortie (FILE *out_stream)
    // Par défaut, on écrit sur l'écran (stdout)
    FILE *out_stream = stdout; 
    
    // Si l'utilisateur a spécifié -o, on ouvre ce fichier
    ...
    if (mode_cfc) {
        // On garde un message sur la console pour l'info utilisateur
    ...
    // 5. Fermeture propre du fichier de sortie
...
// Version Itérative avec FILE *out
void recherche_iter(t_graph *g, t_vertex x, t_vertex y, FILE *out) {
    ...
    if (found) {
        ...
        // Affichage vers le flux 'out'
        while (!stack_is_empty(stack_path_final)) {
            fprintf(out, "%d ", stack_pop(stack_path_final));
        }
        fprintf(out, "\n");
    } else {
        // Message informatif, on le laisse aussi aller vers 'out' ou stderr
...
t_bool Kosaraju_2_recur(t_graph *g, int x, t_bool *marking, FILE *out) {
    ...
    marking[x] = VRAI;
    fprintf(out, "%d ", x); // Écriture dans le fichier
