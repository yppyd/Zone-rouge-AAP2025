 ...
 * Version : LISTES D'ADJACENCE
 * Compilation : gcc filrouge_listes.c -o filrouge_listes
 */
...
typedef struct {
    ...
    t_list * l;    // Tableau de listes d'adjacence
} t_graph;
...
//                                MAIN
// ======================================================================
// Le main reste identique, c'est l'interface utilisateur.
int main(int argc, char *argv[]) {
    ...
    if (input_filename != NULL) {
        ...
        if (in_stream == NULL) { perror("Erreur entree"); return 1; }
    }

    t_graph * g = graph_read_from_file(in_stream);
    if (input_filename != NULL) fclose(in_stream);
    if (g == NULL) { fprintf(stderr, "Erreur lecture graphe.\n"); return 1; }
    ...
    if (mode_cfc) {
        ...
        if (output_filename != NULL) freopen(output_filename, "w", stdout);
        if (start_node >= 0 && goal_node >= 0) recherche_iter(g, start_node, goal_node);
    } else {
        FILE *out_stream = stdout;
        if (output_filename != NULL) out_stream = fopen(output_filename, "w");
...
// ======================================================================
//             IMPLÉMENTATION GRAPHES (LISTES D'ADJACENCE)
// ======================================================================
...
t_graph * graph_new(int size) {
    ...
    // Initialisation de chaque liste à vide (NULL) [cite: 120]
...
// ======================================================================
// (Identique au code fourni, nécessaire pour le fonctionnement)
...
// Version Itérative (Adaptée Listes)
void recherche_iter(t_graph *g, t_vertex x, t_vertex y) {
    ...
    if (found) {
        ...
        while (!stack_is_empty(stack_path_final)) printf("%d ", stack_pop(stack_path_final));
        printf("\n");
    } else {
        printf("Pas de chemin trouve.\n");
    }
    free(marking); free(stack_traversal); free(stack_path); free(stack_path_final);
}
...
t_bool Kosaraju_2_recur(t_graph *g, int x, t_bool *marking) {
    ...
    t_node * curr = g->l[x];
    while (curr != NULL) {
        Kosaraju_2_recur(g, curr->val, marking);
        curr = curr->p_next;
    }
    // -------------------------
